  ...
  <div class="goods-cate-content">
    ...
    <div class="goods-cate-list-container" v-show="showDrag" @click.stop="searchBlur">
      ...
      <div class="goods-cate-footer">
        ...
        <el-button type="text" class="confirm-cate" @click.stop="handaleConfirmCate">确认</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handaleConfirmCate() {
      ...
      let categoryRelStr = ''; //品类关联关系的链
      for (let i in this.allDate) {
        for (let j in this.allDate[i].arr) {
          if (this.allDate[i].arr[j].classIndex !== -1 && this.allDate[i].arr[j].hasOwnProperty('classIndex')) {
            if (categoryRelStr === '') {
              categoryRelStr = this.allDate[i].arr[j].categoryId;
            } else {
              categoryRelStr = categoryRelStr + ',' + this.allDate[i].arr[j].categoryId;
            }
          }
        }
      }
      let params = {
        categoryRelStr: categoryRelStr
      };
      //建立关联关系
      request.post('/api-goods/save-category-group-relation', qs.stringify(params)).then(res => {
        if (res.data.errorCode === 0) {
          categoryRelStr = '';
        } else {
          this.$message.error(res.data.message);
