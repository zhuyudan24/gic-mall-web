<template>
  <el-dialog title="修改商品名称" :visible.sync="editInfoModal.dialogVisible" width="450px" @close="cancelSubmit('editInfoModal')">
  ...
  methods: {
    ...
    submitProNanme(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let params = {
            mallProId: this.editInfoModal.mallProId,
            mallProName: this.editInfoModal.proName
          };
          request.post('/api-mall/update-mall-goods-name', qs.stringify(params)).then(res => {
