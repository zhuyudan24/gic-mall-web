<template>
  <div class="goods-list-content">
    <el-table @sort-change="sortChange" :data="tableData" ref="multipleTable" style="width: 100%" @selection-change="handleSelectAll">
      ...
      <el-table-column prop="upShelvesTime" label="发布时间" sortable="custom" width="110px">
        <template slot-scope="scope">
          <p style="line-height:18px">{{ getTime(scope.row.upShelvesTime) }}</p>
          <p style="line-height:18px" class="gray-color">{{ getSeconds(scope.row.upShelvesTime) }}</p>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="240px" fixed="right">
        <template slot-scope="scope">
          ...
          <tishi ref="tishi" @confirm="deleteItem(scope.row.mallProId, 1, 1)" tips="是否下架该商品?">
    ...
    <!-- 调整库存弹框  -->
    <stockModal :stockModalData="stockModalData" @getList="getList"></stockModal>
    <!-- 修改商品名称 -->
...
export default {
  ...
  methods: {
    ...
    //删除
    deleteItem(mallProId, status, bol, row) {
      // if( bol === -1 ) {
      //   row.soldOutVisible = false;
      // }
      this.$emit('handleItem', mallProId, status, bol);
    },

    cancelDeleteItem(index) {
      for (let item of this.tableData) {
        item.soldOutVisible = false;
      }
      this.tableData[index].soldOutVisible = false;
