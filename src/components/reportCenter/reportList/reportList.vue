    ...
    <div class="mall-content-section">
      ...
      <div class="mall-section">
        <div class="operate-top">
          ...
          <div class="search-item">
            <el-select v-model="dataType" placeholder="所有类型" class="w-120" clearable @change="getListCurr">
              <el-option label="脱敏数据" :value="1"></el-option>
              <el-option label="完整数据" :value="2"></el-option>
            </el-select>
          </div>
          <div class="search-item">
            <el-select v-model="applyStatus" placeholder="所有状态" class="w-120" clearable @change="getListCurr">
              <el-option label="已生成" :value="2"></el-option>
              <el-option label="待审核" :value="1"></el-option>
              <el-option label="生成中" :value="3"></el-option>
              <el-option label="审核超时" :value="4"></el-option>
            </el-select>
          </div>
          <div class="search-item">
            <el-input placeholder="请输入内容" prefix-icon="el-icon-search" class="search-input" @keyup.native.enter="getListCurr" v-model="searchValue">
              <el-select v-model="searchColumn" slot="prepend" placeholder="请选择" class="search-select">
                ...
                <el-option label="操作人员" value="apply_clerk_name"></el-option>
                <el-option label="文件名称" value="report_name"></el-option>
                <el-option label="数据内容" value="data_content"></el-option>
              </el-select>
            </el-input>
          </div>
        </div>
        <div class="list-content">
          <el-table :data="tableData" v-loading="loading">
            ...
            <el-table-column label="操作时间">
              <template slot-scope="scope">
                <p class="time-lh">{{ scope.row.applyTime ? getTime(scope.row.applyTime) : '--' }} {{ scope.row.applyTime ? getSeconds(scope.row.applyTime) : '--' }}</p>
              </template>
            </el-table-column>
            <el-table-column label="操作人员" prop="applyName"> </el-table-column>
            ...
            <el-table-column label="数据总量" prop="dataCount"> </el-table-column>
              ...
              <template slot-scope="scope">
                <p v-if="scope.row.applyStatus == 2" class="dm-status--success">
                  ...
                  <el-tooltip placement="right">
                    <div slot="content">
                      <p>审核人姓名：{{ scope.row.auditorName ? scope.row.auditorName : '--' }}</p>
                      <p>审核人手机：{{ scope.row.auditorPhone ? scope.row.auditorPhone : '--' }}</p>
                    </div>
                    <span>已生成</span>
                  </el-tooltip>
                </p>
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                ...
                <div v-if="scope.row.applyStatus == 2">
                  ...
                  <el-button type="text" @click="reportList(scope.row.reportId)">记录</el-button>
                </div>
                <div v-else>--</div>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      ...
      request.post('/api-admin/page-download-report', qs.stringify(params)).then(res => {
    ...
    getTimeDate(val) {
      if (val) {
        this.applyStartTime = val[0];
        this.applyEndTime = val[1];
        this.currentPage = 1;
      } else {
        this.applyStartTime = '';
        this.applyEndTime = '';
    ...
    // 下载：
    downLoadReport(downloadReportId) {
      // 下载
      let params = {
